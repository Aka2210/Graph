    ...
    for t in range(total_time):
        ...
        for s in srcs:
            for target in sats + clouds:
                ...
                if d <= thr_src_to_sat:
                    G.add_edge(s, target, latency=d, bandwidth=_sample_edge_bw(avg_src_bw, rng))
        ...
        for dn in dests:
            for sl in sats + clouds:
                ...
                if d <= thr_sat_to_dest:
                    G.add_edge(sl, dn, latency=d, bandwidth=_sample_edge_bw(avg_src_bw, rng))
        ...
        for i in range(len(sats)):
            for j in range(i + 1, len(sats)):
                ...
                if dist <= thr_sat_to_sat:
                    ...
                    G.add_edge(a, b, latency=dist, bandwidth=bw)
                    G.add_edge(b, a, latency=dist, bandwidth=bw)
        ...
        for _ in range(int(p_extra * len(nodes) * (len(nodes) - 1))):
            ...
            if u != v and not G.has_edge(u, v):
                G.add_edge(u, v, latency=euclid_latency(pos[u], pos[v]), bandwidth=_sample_edge_bw(avg_src_bw, rng))
...
def main():
    ...
    # loaded_graph_seq = load_graph_sequence_from_txt(path="output_graphs", idx=1)

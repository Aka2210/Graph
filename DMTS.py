...
def LMBBSP_multicast(graph, s, destinations, alpha, c):
    threshold = max(MBBSP.MBBSP_multicast(graph, s, destinations) * (1 - alpha),
                    graph.nodes[s]["bandwidth"])
    ...
    for u, v in H.edges():
        H[u][v]["tmp_w"] = H[u][v].get("cost_traffic", 1.0)

    results = []

    for cand_idx in range(c):
        pred, dist = nx.dijkstra_predecessor_and_distance(H, source=s, weight="tmp_w")

        if any(d not in pred or not pred.get(d) for d in destinations):
            break
